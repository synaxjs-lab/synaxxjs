...
export const CallModal: React.FC<CallModalProps> = ({ callState, onClose }) => {
  ...
  useEffect(() => {
    ...
    } else if (status === 'calling' && callState.role === 'caller') {
  ...
  // Fail stale unanswered calls instead of leaving a permanent "ongoing" call UI.
  useEffect(() => {
    if (status !== 'calling' && status !== 'ringing') return;
    const timeoutMs = callState.role === 'caller' ? 45_000 : 60_000;
    const timer = window.setTimeout(() => {
      ...
      setStatus('ended');
      setConnectionMessage(callState.role === 'caller' ? 'No answer' : 'Call expired');
      window.setTimeout(onClose, 900);
    }, timeoutMs);
    return () => window.clearTimeout(timer);
  }, [status, callState.callId, callState.role, callState.callType, onClose]);
  ...
  const initiateCallerMedia = async (rtc: WebRTCManager) => {
    try {
      ...
      if (!sent) throw new Error('The call could not be sent to the other person.');
    } catch (err: any) {
  ...
  const handleAcceptCall = async () => {
    ...
    if (!rtcRef.current) {
      setHasError('WebRTC is not ready yet. Please wait a moment and try again.');
      return;
    }
    setIsAccepting(true);
    try {
  ...
  const enableAudio = async () => {
    ...
    try {
      ...
      if (AudioCtx) {
        ...
        window.setTimeout(() => { try { void context.close(); } catch {} }, 250);
      }
    } catch {}
    ...
    try {
      ...
      console.warn('Speaker could not be enabled:', err);
      setAudioBlocked(true);
    }
  };

  const handleRejectCall = () => {
    void socketService.sendCallSignal({ callId: callState.callId, type: 'call:reject', callType: callState.callType });
    setStatus('ended');
    onClose();
  };

  const handleEndCall = () => {
    void socketService.sendCallSignal({ callId: callState.callId, type: 'call:end', callType: callState.callType });
            ...
            <div className="flex flex-col items-center justify-center gap-6 text-center z-10 px-4">
              ...
              {audioBlocked && (
                <button
                  type="button"
                  onClick={enableAudio}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold border border-indigo-400/40"
                >
          ...
          {callState.role === 'callee' && status === 'ringing' ? (
            <div className="flex items-center gap-6">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 disabled:cursor-wait text-white font-semibold text-sm shadow-[0_0_25px_rgba(16,185,129,0.5)] transition-all active:scale-95"
              >
                <Phone className="w-5 h-5" />
                <span>
                  {isAccepting ? 'Connecting…' : <>Accept {displayOtherUserName}'s{' '}
