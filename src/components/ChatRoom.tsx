...
export const ChatRoom: React.FC<ChatRoomProps> = ({
  ...
  return (
    <div
      id="synax-chat-room"
      className="relative z-10 w-full h-screen flex flex-col bg-[#04060c]/80 backdrop-blur-sm text-slate-100 overflow-hidden select-none"
    >
      {/* Top Banner (Authoritative Server Time System) */}
      <TimeBanner timeStatus={timeStatus} />

      {/* Main Header Bar */}
      <header className="w-full px-4 sm:px-6 py-3 bg-slate-950/90 border-b border-slate-800/80 flex items-center justify-between z-20 backdrop-blur-xl">
        {/* Left: Other Person Profile & Presence */}
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <span
              className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-slate-950 ${
                otherPresence.isOnline ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-slate-500'
              }`}
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-sm sm:text-base font-bold text-white font-cinzel tracking-anime-header text-shadow-anime-glow">
                {otherUser.nickname || otherUser.name}
              </h2>
              {/* Subtle vector mark of other person */}
              <div title={`Personal Mark: ${otherUser.logo.name}`}>
                <LogoMark logo={otherUser.logo} size="sm" glow={false} className="opacity-70 hover:opacity-100" />
              </div>
            </div>
        ...
        {/* Right: Calling Controls, Search, Profile, Time & Logout */}
